...
@Component({
  selector: 'app-purchaseordermodule',
  standalone: true,
  imports: [RouterModule, CommonModule, NgbModalModule, WebdtableComponent],
  templateUrl: './purchaseordermodule.component.html',
  styleUrl: './purchaseordermodule.component.scss'
})
export class PurchaseordermoduleComponent {
  ...
  getOderDetails() {
    this._base._encryptedStorage.get(enAppSession.user_id).then((user_id: any) => {
      ...
      this._webDService.getpurchaseorder('all', 0, user_id, parseInt(start), parseInt(end)).subscribe((resquoteDetails: any) => {
        this.quoteDetails = [];
