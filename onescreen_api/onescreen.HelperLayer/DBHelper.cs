...
namespace webdHelper
{
    public class DBHelper
    {
        ...
        public string BulkImport(string TableName, DataTable DataTable)
        {
            string Response = "";
            ...
            try
            {
                SqlBulkCopy objbulk = new SqlBulkCopy(_con);
                objbulk.DestinationTableName = AppendTimeStamp(TableName);
                _con.Open();
                var checkTableIfExistsCommand = new SqlCommand("IF EXISTS (SELECT 1 FROM sysobjects WHERE name =  '" + objbulk.DestinationTableName + "') SELECT 1 ELSE SELECT 0", _con);
                var exists = checkTableIfExistsCommand.ExecuteScalar().ToString().Equals("1");
                // if does not exist
                if (!exists)
                {
                    var createTableBuilder = new StringBuilder("CREATE TABLE [" + objbulk.DestinationTableName + "]");
                    ...
                    var createTableCommand = new SqlCommand(createTableBuilder.ToString(), _con);
                    createTableCommand.ExecuteNonQuery();
                }
                objbulk.WriteToServer(DataTable);
                _con.Close();
                Response = objbulk.DestinationTableName;
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        //public string Bulk_Import(string TableName, DataTable DataTable)
        //{
        //    string Response = "";
        //    string connectionString = Configuration.ConnectionString;
        //    SqlConnection _con = new SqlConnection(connectionString);
        ...
        //        objbulk.WriteToServer(DataTable);
        //        _con.Close();
        //        Response = "ImportProceed";
        //    }
        //    catch (Exception ex)
        //    {
        //        Response = "ImportFailed";
        //    }
        //    finally
        //    {
        //        _con.Close();
        ...
        public string AppendTimeStamp(string tablename)
        {
